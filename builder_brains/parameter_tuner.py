...
_VECTOR_TOLERANCE: float = 1e-09
_CONFIG_TOLERANCE: float = 1e-09
_ACCURACY_KEYS: Tuple[str, ...] = ('accuracy_score', 'accuracy', 'score', 'success_rate', 'precision')
_WALL_SECONDS_KEYS: Tuple[str, ...] = ('wall_seconds', 'execution_time', 'execution_seconds', 'elapsed_seconds', 'duration_seconds', 'runtime_seconds')
_WALL_MILLIS_KEYS: Tuple[str, ...] = ('latency_ms', 'wall_ms', 'duration_ms')
_WALL_MICROS_KEYS: Tuple[str, ...] = ('latency_us', 'wall_us', 'duration_us')
_COMPRESSION_KEYS: Tuple[str, ...] = ('compression_ratio', 'compression', 'compaction_ratio', 'minify_compression')
...
def measurement_to_objectives(accuracy: Any, wall_seconds: Any, compression: Any) -> List[float]:
    ...
    return coerce_objective_vector([max(0.0, accuracy_value) if accuracy_value is not None else 0.0, safe_inverse_wall_seconds(wall_seconds), max(0.0, compression_value) if compression_value is not None else 0.0])
...
def parse_performance_record(record: Any) -> List[float]:
    ...
    if isinstance(record, dict):
        ...
        if wall_seconds is None:
            ...
            wall_seconds = millis / 1000.0 if millis is not None else None
        if wall_seconds is None:
            micros = _first_numeric(record, _WALL_MICROS_KEYS)
            wall_seconds = micros / 1000000.0 if micros is not None else None
...
def _synthetic_objective_vector() -> List[float]:
    ...
    return measurement_to_objectives(random.uniform(0.3, 0.9), random.uniform(0.05, 2.5), random.uniform(0.1, 0.9))

def collect_objective_vectors(metadata: Dict[str, Any], count: int) -> Tuple[List[List[float]], Dict[str, int]]:
    ...
    """
    sources = {'performance_log': 0, 'fitness_matrix': 0, 'legacy_scalars': 0, 'synthetic': 0}
            ...
            if isinstance(rows, (list, tuple)):
                for row in list(rows)[:count - len(vectors)]:
                    if isinstance(row, (list, tuple)):
                        ...
                        vectors.append(measurement_to_objectives(padded[0], padded[1], padded[2]))
...
def _vectors_close(vector_a: Sequence[float], vector_b: Sequence[float], tolerance: float=_VECTOR_TOLERANCE) -> bool:
    if len(vector_a) != len(vector_b):
        return False
    return all((abs(a - b) <= tolerance for a, b in zip(vector_a, vector_b)))

def _configs_close(config_a: Dict[str, float], config_b: Dict[str, float], tolerance: float=_CONFIG_TOLERANCE) -> bool:
...
class AnnealingSchedule:
    ...
    def __init__(self, initial_lr: float=0.01, min_lr: float=1e-05, warmup_cycles: int=50, restart_period: int=100, restart_multiplier: float=2.0) -> None:
    ...
    def step_decay(self, cycle: int, drop_factor: float=0.5, drop_every: int=100) -> float:
        if cycle < self.warmup_cycles:
            return self._linear_warmup(cycle)
        effective = cycle - self.warmup_cycles
    ...
    def get_lr(self, cycle: int, schedule_name: str='cosine_warm_restart') -> float:
        dispatch: Dict[str, Any] = {'cosine_warm_restart': self.cosine_warm_restart, 'exponential_decay': self.exponential_decay, 'step_decay': self.step_decay}
...
class ParameterBound:
    ...
    def __init__(self, name: str, low: float, high: float, log_scale: bool=False, dtype: str='float') -> None:
        self.name: str = name
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'name': self.name, 'low': self.low, 'high': self.high, 'log_scale': self.log_scale, 'dtype': self.dtype}
...
class HyperparameterMatrix:
    ...
    def add_parameter(self, name: str, low: float, high: float, log_scale: bool=False, dtype: str='float') -> None:
        self._bounds[name] = ParameterBound(name=name, low=low, high=high, log_scale=log_scale, dtype=dtype)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'dimension': self.dimension, 'parameters': {n: b.to_dict() for n, b in self._bounds.items()}}

def build_default_matrix() -> HyperparameterMatrix:
    matrix = HyperparameterMatrix()
    matrix.add_parameter('learning_rate', 1e-05, 1.0, log_scale=True)
    ...
    matrix.add_parameter('elite_ratio', 0.05, 0.5)
    matrix.add_parameter('exploration_epsilon', 0.01, 0.5)
    matrix.add_parameter('discount_gamma', 0.8, 0.9999)
    return matrix
...
class Individual:
    ...
    def __init__(self, config: Dict[str, float], fitness: Optional[Sequence[float]]=None, generation: int=0, parent_id: Optional[int]=None) -> None:
    ...
    def clone(self, generation: int) -> 'Individual':
        ...
        child = Individual(config=dict(self.config), fitness=list(self.fitness), generation=generation, parent_id=self.uid)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'uid': self.uid, 'config': {k: round(v, 6) for k, v in self.config.items()}, 'fitness': [round(v, 6) for v in self.fitness], 'objectives': {k: round(v, 6) for k, v in self.objectives.items()}, 'rank': self.rank, 'crowding_distance': round(crowding, 6) if math.isfinite(crowding) else 'inf', 'generation': self.generation, 'parent_id': self.parent_id}
    ...
    for objective_idx in range(NUM_OBJECTIVES):
        ...
        for position in range(1, count - 1):
            ...
            if math.isinf(individual.crowding_distance):
                continue
            gap = ordered[position + 1].fitness[objective_idx] - ordered[position - 1].fitness[objective_idx]
...
def hypervolume(points: List[Sequence[float]], reference: Sequence[float]) -> float:
    ...
    if dimension == 2:
        return _hypervolume_2d([(point[0], point[1]) for point in points], (reference[0], reference[1]))
    ref_x, ref_y, ref_z = (reference[0], reference[1], reference[2])
    candidates = [(point[0], point[1], point[2]) for point in points if point[0] > ref_x and point[1] > ref_y and (point[2] > ref_z)]
    if not candidates:
        return 0.0
...
class FitnessSurvivalTracker:
    ...
    def __init__(self, archive_capacity: int=64, history_window: int=100, ema_alpha: float=0.1, stagnation_patience: int=200, min_improvement_delta: float=0.0005, reference_point: Sequence[float]=(0.0,) * NUM_OBJECTIVES, initial_ema_hypervolume: float=0.0, initial_best_hypervolume: float=0.0, initial_stagnation_cycles: int=0) -> None:
        ...
        self._reference_point: Tuple[float, ...] = tuple((float(value) for value in padded_reference))
    ...
    def _merge_into_archive(self, candidates: List[Individual]) -> bool:
        ...
        for individual in self._archive:
            if not any((_vectors_close(individual.fitness, previous) for previous in previous_vectors)):
                return True
        return False
    ...
    def restore_archive(self, records: Any) -> int:
        ...
        for record in records:
            ...
            restored.append(Individual(config=config, fitness=coerce_objective_vector(record.get('fitness')), generation=int(generation) if generation is not None else 0))
        if restored:
    ...
    def update_frontier(self, candidates: List[Individual]) -> List[Individual]:
        ...
        current_hv = hypervolume([ind.fitness for ind in self._archive], self._reference_point)
        self._hypervolume_history.append(current_hv)
        self._ema_hypervolume = self._ema_alpha * current_hv + (1.0 - self._ema_alpha) * self._ema_hypervolume
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {'total_generations': self._total_generations, 'frontier_size': len(self._archive), 'archive_capacity': self._archive_capacity, 'hypervolume': round(recent[-1] if recent else 0.0, 6), 'best_hypervolume': round(self._best_hypervolume, 6), 'ema_hypervolume': round(self._ema_hypervolume, 6), 'objective_bests': {k: round(v, 6) for k, v in objective_bests.items()}, 'cycles_without_improvement': self._cycles_without_improvement, 'is_stagnant': self.is_stagnant, 'history_size': len(recent), 'recent_mean_hypervolume': round(sum(recent) / max(len(recent), 1), 6), 'recent_max_hypervolume': round(max(recent) if recent else 0.0, 6), 'recent_min_hypervolume': round(min(recent) if recent else 0.0, 6)}
...
def gaussian_mutate(config: Dict[str, float], sigma: float, matrix: HyperparameterMatrix) -> Dict[str, float]:
...
def uniform_crossover(parent_a: Dict[str, float], parent_b: Dict[str, float], probability: float=0.5) -> Dict[str, float]:
...
def blend_crossover(parent_a: Dict[str, float], parent_b: Dict[str, float], alpha: float=0.5) -> Dict[str, float]:
    child: Dict[str, float] = {}
    for key in set(parent_a.keys()) | set(parent_b.keys()):
...
def _objective_aggregates(population: List[Individual]) -> Dict[str, Dict[str, float]]:
    ...
    for idx, name in enumerate(OBJECTIVE_NAMES):
        ...
        if values:
            aggregates[name] = {'mean': round(sum(values) / len(values), 6), 'max': round(max(values), 6), 'min': round(min(values), 6)}
        else:
...
class EvolutionaryEngine:
    ...
    def __init__(self, matrix: HyperparameterMatrix, population_size: int=64, mutation_sigma: float=0.05, crossover_prob: float=0.7, tournament_k: int=3, offspring_reserve: float=0.125) -> None:
    ...
    def inject_configurations(self, configs: Any, from_tail: bool=False) -> int:
        ...
        if from_tail:
            ...
            for offset, config in enumerate(usable):
                ...
                if slot < 0:
                    break
                self._population[slot] = Individual(config=self._matrix.clip_configuration(config), generation=self._generation)
                injected += 1
            return injected
    ...
    def evolve(self, tracker: FitnessSurvivalTracker) -> List[Individual]:
        ...
        next_generation: List[Individual] = [member.clone(self._generation) for member in survivors]
        while len(next_generation) < self._pop_size:
            ...
            next_generation.append(Individual(config=child_config, generation=self._generation, parent_id=parent_a.uid))
    ...
    def population_stats(self) -> Dict[str, Any]:
        if not self._population:
            return {'size': 0}
        return {'size': len(self._population), 'generation': self._generation, 'front_count': self._last_front_count, 'pareto_frontier_size': self._last_frontier_size, 'objective_aggregates': self._last_pool_aggregates or _objective_aggregates(self._population)}

def detect_parameter_drift(current_config: Dict[str, float], baseline_config: Dict[str, float], max_drift: float=0.3) -> Dict[str, Any]:
    ...
    for key in set(current_config.keys()) | set(baseline_config.keys()):
        ...
        denominator = abs(baseline_val) if abs(baseline_val) > 1e-09 else 1.0
        drift = abs(current_val - baseline_val) / denominator
        total_drift += drift
        param_count += 1
        if drift > max_drift:
            drifted_params.append({'parameter': key, 'current': round(current_val, 6), 'baseline': round(baseline_val, 6), 'drift_ratio': round(drift, 6)})
...
def evaluate(metadata: Dict[str, Any], hyper_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    elite_ratio: float = float(params.get('elite_survival_ratio', 0.2))
    mutation_sigma: float = float(params.get('mutation_sigma', 0.05))
    crossover_prob: float = float(params.get('crossover_probability', 0.7))
    ...
    max_drift: float = float(thresholds.get('parameter_drift_max', 0.3))
    ...
    if metadata.get('boost_mutation_sigma'):
        ...
        logger.info('Kinetic stagnation signal honored: mutation sigma boosted to %.4f', mutation_sigma)
    metadata['mutation_sigma_boosted'] = sigma_boosted
    scheduler = AnnealingSchedule(initial_lr=initial_lr, min_lr=min_lr, warmup_cycles=warmup_cycles, restart_period=100, restart_multiplier=restart_mult)
    ...
    logger.info('Learning rate annealed: cycle=%d, lr=%.8f, schedule=%s', current_cycle, annealed_lr, schedule_name)
    ...
    elapsed = time.monotonic() - start_time
    if elapsed < wall_limit:
        engine = EvolutionaryEngine(matrix=matrix, population_size=pop_size, mutation_sigma=mutation_sigma, crossover_prob=crossover_prob, tournament_k=tournament_k)
        archive_capacity = max(pop_size, int(round(pop_size * (1.0 + max(0.0, elite_ratio)))))
        previous_stats = metadata.get('survival_tracker_stats')
        if not isinstance(previous_stats, dict):
            previous_stats = {}
        tracker = FitnessSurvivalTracker(archive_capacity=archive_capacity, history_window=history_window, ema_alpha=ema_alpha, stagnation_patience=stagnation_patience, min_improvement_delta=min_improvement, initial_ema_hypervolume=_coerce_float(previous_stats.get('ema_hypervolume')) or 0.0, initial_best_hypervolume=_coerce_float(previous_stats.get('best_hypervolume')) or 0.0, initial_stagnation_cycles=int(_coerce_float(previous_stats.get('cycles_without_improvement')) or 0))
        restored_frontier = tracker.restore_archive(metadata.get('pareto_frontier'))
        restored_population = engine.inject_configurations(metadata.get('tuned_population'))
        ...
        if isinstance(bootstrap_configs, (list, tuple)) and bootstrap_configs:
            bootstrap_injected = engine.inject_configurations(bootstrap_configs, from_tail=True)
            metadata['bootstrap_seed_configs'] = []
            metadata['warm_bootstrap_consumed'] = bootstrap_injected > 0
            logger.info('Warm bootstrap: re-seeded %d population slots from replay trajectories', bootstrap_injected)
        metadata['warm_bootstrap_injected'] = bootstrap_injected
        fitness_vectors, vector_sources = collect_objective_vectors(metadata, len(engine.population))
        ...
        metadata['elite_configurations'] = [{key: round(value, 6) for key, value in ind.config.items()} for ind in frontier]
        metadata['tuned_population'] = [{key: round(value, 6) for key, value in ind.config.items()} for ind in new_population]
        ...
        logger.info('Pareto evolution: gen=%d, frontier=%d, hypervolume=%.6f, sources=%s', engine.generation, len(frontier), metadata['survival_tracker_stats']['hypervolume'], vector_sources)
    else:
    ...
    if elapsed < wall_limit:
        ...
        metadata['convergence_check'] = {'hypervolume': round(current_hv, 6), 'ema_hypervolume': round(ema_hv, 6), 'gap': round(gap, 6), 'relative_gap': round(relative_gap, 6), 'tolerance': convergence_tol, 'converged': converged}
        if converged:
            logger.info('Convergence detected — relative hypervolume gap %.6f < tolerance %.6f', relative_gap, convergence_tol)
    metadata['mutation_rate'] = mutation_sigma
    metadata['annealing_temp'] = round(initial_lr / math.log1p(current_cycle + 1), 8)
