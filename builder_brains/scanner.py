...
class TokenNgramAnalyzer:
    ...
    def build_sequence_from_positions(self, positions: Dict[str, List[Tuple[int, int]]]) -> List[str]:
    ...
    def analyze(self, source: str, profiler: TokenProfiler) -> Dict[str, Any]:
        ...
        return {'total_ngrams': total_ngrams, 'unique_ngrams': len(ngrams), 'top_ngrams': [{'ngram': list(ng), 'count': ct} for ng, ct in top_ngrams], 'sequence_length': len(sequence)}
...
class CryptoDeltaDiffer:
    ...
    def compute_delta(self, old_content: str, new_content: str) -> Dict[str, Any]:
        ...
        return {'changed': changed, 'old_fingerprint': old_fp, 'new_fingerprint': new_fp, 'old_line_count': len(old_lines), 'new_line_count': len(new_lines), 'lines_added': added_count, 'lines_removed': removed_count, 'size_delta_bytes': size_delta}

    def batch_fingerprint(self, file_contents: Dict[str, str]) -> Dict[str, str]:
        result: Dict[str, str] = {}
        for path, content in file_contents.items():
            result[path] = self.fingerprint(content)
        return result

    def detect_duplicates(self, file_fingerprints: Dict[str, str]) -> Dict[str, List[str]]:
...
class AnomalyCategory:
    ...
    def __init__(self, category: str, severity: str, zscore: float, metric_name: str, metric_value: float, context: str='') -> None:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'category': self.category, 'severity': self.severity, 'zscore': round(self.zscore, 4), 'metric_name': self.metric_name, 'metric_value': round(self.metric_value, 4), 'context': self.context}
...
class AnomalyCategorizer:
    ...
    """
    CATEGORY_RULES: Dict[str, Dict[str, Any]] = {'complexity_spike': {'metrics': ['function_def', 'class_def'], 'description': 'Unusually high definition density'}, 'import_bloat': {'metrics': ['import_stmt'], 'description': 'Excessive import statements'}, 'comment_desert': {'metrics': ['comment_line'], 'description': 'Abnormally low comment density', 'invert': True}, 'error_handling_deficit': {'metrics': ['try_block', 'except_block'], 'description': 'Insufficient error handling', 'invert': True}, 'magic_number_overload': {'metrics': ['numeric_literal'], 'description': 'Excessive magic numbers'}}
        ...
        for category, rule in self.CATEGORY_RULES.items():
            ...
            for metric_name in metric_names:
                ...
                if is_anomalous:
                    ...
                    anomalies.append(AnomalyCategory(category=category, severity=severity, zscore=zscore, metric_name=metric_name, metric_value=value, context=rule.get('description', '')))
        return anomalies
...
class FileScanner:
    ...
    def __init__(self, worker_count: int=8, max_file_bytes: int=10485760) -> None:
    ...
    def _scan_single_file(self, file_path: str) -> Tuple[Dict[str, Any], Optional[Dict[str, str]]]:
    ...
    def scan_files(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=self._worker_count) as executor:
            future_to_path: Dict[Future[Tuple[Dict[str, Any], Optional[Dict[str, str]]]], str] = {}
            ...
            for future in as_completed(future_to_path):
                ...
                try:
                    ...
                    results.append({'path': path, 'status': 'thread_failure', 'error': str(exc)})
        ...
        return {'results': results, 'scanned_count': scanned_count, 'failed_count': failed_count, 'total_files': len(file_paths), 'scan_duration_seconds': scan_duration, 'errors': errors}

    def collect_profiles(self, scan_results: Dict[str, Any]) -> Tuple[List[Dict[str, int]], Dict[str, str]]:
...
def _cosine_similarity(vec_a: Dict[str, int], vec_b: Dict[str, int]) -> float:
    ...
    mag_a = math.sqrt(sum((v ** 2 for v in vec_a.values()))) or 1e-09
    mag_b = math.sqrt(sum((v ** 2 for v in vec_b.values()))) or 1e-09
    return dot / (mag_a * mag_b)

def find_structurally_similar_files(scan_results: Dict[str, Any], cutoff: float=0.8) -> List[Dict[str, Any]]:
    ...
    for i in range(len(entries)):
        for j in range(i + 1, len(entries)):
            ...
            if sim >= cutoff:
                similar_pairs.append({'file_a': entries[i]['path'], 'file_b': entries[j]['path'], 'similarity': round(sim, 6)})
...
def evaluate(metadata: Dict[str, Any], hyper_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info('Scan complete: %d/%d files (%.1f%% coverage)', scan_output['scanned_count'], scan_output['total_files'], coverage * 100)
    ...
    if elapsed < wall_limit:
        ...
        if previous_fps:
            ...
            metadata['delta_summary'] = {'changed_files': changed_files, 'new_files': new_files, 'removed_files': removed_files, 'changed_count': len(changed_files), 'new_count': len(new_files), 'removed_count': len(removed_files)}
            logger.info('Delta diff: %d changed, %d new, %d removed', len(changed_files), len(new_files), len(removed_files))
            duplicates = differ.detect_duplicates(fingerprints)
            if duplicates:
                metadata['duplicate_groups'] = {fp: paths for fp, paths in duplicates.items()}
    ...
    if elapsed < wall_limit and profiles:
        ...
        if len(all_anomalies) > anomaly_ceiling:
            logger.warning('Anomaly count %d exceeds ceiling %d — truncating', len(all_anomalies), anomaly_ceiling)
            all_anomalies = sorted(all_anomalies, key=lambda a: abs(a.get('zscore', 0)), reverse=True)[:anomaly_ceiling]
        ...
        logger.info('Anomaly detection: %d anomalies found (%s)', len(all_anomalies), dict(severity_counts))
    else:
        metadata['anomalies'] = []
    ...
    if elapsed < wall_limit and profiles:
        ...
        for scan_entry in scan_output.get('results', []):
            ...
            try:
                ...
                ngram_summaries.append({'file': path, 'total_ngrams': ngram_result['total_ngrams'], 'unique_ngrams': ngram_result['unique_ngrams']})
            except Exception as exc:
