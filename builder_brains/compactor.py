...
class StructuralTokenizer(ast.NodeVisitor):
    ...
    def generic_visit(self, node: ast.AST) -> None:
        ...
        if self._current_depth >= self.max_depth:
            logger.debug('Max tree-walk recursion depth %d reached at node %s', self.max_depth, node_type_name)
            return
    ...
    def get_depth_stats(self) -> Dict[str, int]:
        return {'max_observed_depth': self._max_observed_depth, 'total_nodes': sum(self.node_counts.values()), 'unique_node_types': len(self.node_counts)}
...
def tokenize_source(source: str, max_depth: int=256) -> Dict[str, Any]:
    ...
    return {'token_vector': tokenizer.get_token_vector(), 'depth_stats': tokenizer.get_depth_stats()}
...
class DeadCodeEliminator(ast.NodeTransformer):
    ...
    def visit_Import(self, node: ast.Import) -> Optional[ast.Import]:
        remaining = [alias for alias in node.names if (alias.asname or alias.name) in self._loaded_names]
        if not remaining:
            self.removed_nodes += 1
            return None
...
def eliminate_dead_code(source: str, depth: int=4) -> Dict[str, Any]:
    tree = ast.parse(source)
    ...
    return {'cleaned_source': cleaned_source, 'original_node_count': original_node_count, 'final_node_count': final_node_count, 'removed_node_count': eliminator.removed_nodes, 'reduction_ratio': 1.0 - final_node_count / max(original_node_count, 1)}
...
class ASTTreeTrimmer(ast.NodeTransformer):
    ...
    def __init__(self, confidence_floor: float=0.6, aggressiveness: float=0.72) -> None:
...
def trim_ast_tree(source: str, confidence_floor: float=0.6, aggressiveness: float=0.72) -> Dict[str, Any]:
    tree = ast.parse(source)
    original_count = sum((1 for _ in ast.walk(tree)))
    trimmer = ASTTreeTrimmer(confidence_floor=confidence_floor, aggressiveness=aggressiveness)
    tree = trimmer.visit(tree)
    ast.fix_missing_locations(tree)
    trimmed_source = ast.unparse(tree)
    final_count = sum((1 for _ in ast.walk(ast.parse(trimmed_source))))
    return {'trimmed_source': trimmed_source, 'original_node_count': original_count, 'final_node_count': final_count, 'trimmed_branches': trimmer.trimmed_branches}

class _ScopeAnalyzer(ast.NodeVisitor):
    """Build a mapping of locally-defined names per function scope."""
    BUILTIN_NAMES: Set[str] = {'print', 'len', 'range', 'enumerate', 'zip', 'map', 'filter', 'int', 'float', 'str', 'bool', 'list', 'dict', 'set', 'tuple', 'isinstance', 'issubclass', 'type', 'super', 'property', 'staticmethod', 'classmethod', 'open', 'hasattr', 'getattr', 'setattr', 'delattr', 'None', 'True', 'False', 'Exception', 'ValueError', 'TypeError', 'KeyError', 'IndexError', 'RuntimeError', 'OSError', 'IOError', 'AttributeError', 'ImportError', 'StopIteration', 'NotImplementedError', 'AssertionError'}
...
class VariableMinifier(ast.NodeTransformer):
    ...
    def __init__(self, scope_map: Dict[str, Set[str]], entropy_cap: float=0.85, salt_bits: int=32) -> None:
...
def minify_variables(source: str, entropy_cap: float=0.85, salt_bits: int=32) -> Dict[str, Any]:
    tree = ast.parse(source)
    analyzer = _ScopeAnalyzer()
    analyzer.visit(tree)
    minifier = VariableMinifier(scope_map=dict(analyzer.scopes), entropy_cap=entropy_cap, salt_bits=salt_bits)
    ...
    return {'minified_source': minified_source, 'total_renames': minifier.total_renames, 'original_length': original_len, 'minified_length': minified_len, 'compression_ratio': round(compression, 6)}
...
def evaluate(metadata: Dict[str, Any], hyper_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info('Tokenization complete — %d unique types, depth %d', tok_result['depth_stats']['unique_node_types'], tok_result['depth_stats']['max_observed_depth'])
    ...
    if elapsed < wall_limit:
        try:
            ...
            logger.info('Dead-code elimination: removed %d nodes (%.2f%% reduction)', dce_result['removed_node_count'], dce_result['reduction_ratio'] * 100)
    ...
    if elapsed < wall_limit:
        try:
            confidence_floor: float = float(params.get('node_prune_confidence_floor', 0.6))
            aggressiveness: float = float(params.get('ast_trim_aggressiveness', 0.72))
            trim_result = trim_ast_tree(source, confidence_floor=confidence_floor, aggressiveness=aggressiveness)
            ...
            logger.info('AST trim: pruned %d low-confidence branches', trim_result['trimmed_branches'])
        except Exception as exc:
    ...
    if elapsed < wall_limit:
        ...
        if current_entropy <= entropy_cap:
            try:
                ...
                min_result = minify_variables(source, entropy_cap=entropy_cap, salt_bits=salt_bits)
                ...
                logger.info('Variable minification: %d renames, %.2f%% compression', min_result['total_renames'], min_result['compression_ratio'] * 100)
            except Exception as exc:
                logger.error('Variable minification failed: %s', exc)
                metadata['minify_error'] = str(exc)
        else:
            logger.info('Skipping minification — entropy %.4f exceeds cap %.4f', current_entropy, entropy_cap)
    ...
    compaction_floor: float = float(thresholds.get('compaction_ratio_floor', 0.1))
