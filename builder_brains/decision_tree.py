...
class EvaluationItem:
    ...
    def __init__(self, score: float, cost: float, label: str, payload: Dict[str, Any]) -> None:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'label': self.label, 'score': round(self.score, 6), 'cost': round(self.cost, 6), 'net_value': round(self.net_value, 6)}
...
class EvaluationPriorityQueue:
    ...
    def stats(self) -> Dict[str, int]:
        return {'capacity': self._capacity, 'current_size': self.size, 'total_pushed': self._total_pushed, 'total_evicted': self._total_evicted}
...
class CostBenefitAnalyzer:
    ...
    def net_benefit(self, raw_score: float, raw_cost: float, step: int, transitions: int) -> float:
        return self.discounted_score(raw_score, step) - self.penalized_cost(raw_cost, transitions)

    def rank_actions(self, actions: List[Dict[str, Any]], current_step: int, current_transitions: int) -> List[Dict[str, Any]]:
...
class HeuristicState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'name': self.name, 'layer': self.layer, 'entry_count': self.entry_count, 'average_score': round(self.average_score, 6), 'transitions': list(self.transitions.keys())}
...
class HeuristicStateMachine:
    ...
    def __init__(self, layer_count: int=5, velocity_epsilon: float=0.005, acceleration_epsilon: float=0.005, stall_patience: int=KINETIC_STALL_PATIENCE) -> None:
    ...
    def _build_layers(self, layer_count: int) -> None:
        layer_names = ['conservative_scan', 'exploratory_probe', 'greedy_exploit', 'balanced_adaptive', 'fallback_cascade']
    ...
    def restore_kinetic_state(self, stall_cycles: Any) -> None:
        ...
        self._kinetic_anomaly_flagged = self._kinetic_stall_cycles >= self._stall_patience
    ...
    def record_score(self, score: float) -> Dict[str, Any]:
        ...
        if len(history) >= 3 and abs(velocity) < self._velocity_epsilon and (abs(acceleration) < self._acceleration_epsilon):
            self._kinetic_stall_cycles += 1
        else:
            self._kinetic_stall_cycles = 0
        self._kinetic_anomaly_flagged = self._kinetic_stall_cycles >= self._stall_patience
        if self._kinetic_anomaly_flagged:
            logger.warning('Kinetic stagnation anomaly: velocity=%.6f acceleration=%.6f stalled for %d consecutive cycles', velocity, acceleration, self._kinetic_stall_cycles)
    ...
    def kinetic_snapshot(self) -> Dict[str, Any]:
        return {'history': [round(v, 6) for v in self._score_history], 'window_capacity': self.KINETIC_WINDOW, 'velocity': round(self._latest_velocity, 6), 'acceleration': round(self._latest_acceleration, 6), 'velocity_epsilon': self._velocity_epsilon, 'acceleration_epsilon': self._acceleration_epsilon, 'stall_cycles': self._kinetic_stall_cycles, 'stall_patience': self._stall_patience, 'stagnation_anomaly': self._kinetic_anomaly_flagged}
    ...
    def transition(self, signal: str, score: float) -> str:
        ...
        if target is None:
            logger.warning("No transition for signal '%s' in state '%s' — holding", signal, self._current.name)
            target = self._current
        previous_name = self._current.name
        self._current = target
        self._current.enter(score)
        self._transition_count += 1
        log_entry = {'step': self._transition_count, 'from': previous_name, 'signal': signal, 'to': self._current.name, 'score': round(score, 6)}
        self._transition_log.append(log_entry)
        logger.debug('Transition: %s', log_entry)
        return self._current.name

    def decide_signal(self, score: float, exploration_epsilon: float, exploitation_bias: float, confidence_minimum: float) -> str:
        if self._kinetic_anomaly_flagged:
            logger.info('Kinetic stagnation interception gate tripped — emitting boost_mutation_sigma to expand search variance')
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {'current_state': self._current.to_dict() if self._current else None, 'transition_count': self._transition_count, 'states': {n: s.to_dict() for n, s in self._states.items()}, 'recent_transitions': self._transition_log[-10:], 'kinetics': self.kinetic_snapshot()}
...
class FallbackRouter:
    ...
    def __init__(self, max_depth: int=6, ttl_seconds: float=300.0, replay_buffer: Optional['ExperienceReplayBuffer']=None) -> None:
        self._max_depth: int = max(1, max_depth)
        self._ttl: float = ttl_seconds
        self._cache: OrderedDict[str, Tuple[Strategy, float]] = OrderedDict()
        self._cascade_order: List[Strategy] = [Strategy.GREEDY, Strategy.EXPLORATORY, Strategy.BALANCED, Strategy.CONSERVATIVE, Strategy.FALLBACK_CASCADE, Strategy.RANDOM_RESTART]
    ...
    def record_experience(self, state: Dict[str, Any], action: str, result: float, parameter_matrix: Optional[Dict[str, float]]=None) -> bool:
        """Persist a state-action-result triplet for future warm bootstraps."""
        if self._replay_buffer is None:
            return False
        try:
            return self._replay_buffer.record_trajectory(state, action, result, parameter_matrix=parameter_matrix)
        except Exception as exc:
    ...
    def _attempt_warm_bootstrap(self, metadata: Dict[str, Any]) -> bool:
        ...
        if self._replay_buffer is None:
            return False
        try:
            query_state = {'current_score': float(metadata.get('current_score', 0.0) or 0.0), 'current_cycle': float(metadata.get('current_cycle', 0) or 0), 'fsm_state': str(metadata.get('fsm_state', '')), 'primary_strategy': str(metadata.get('primary_strategy', ''))}
            matrices = self._replay_buffer.best_parameter_matrices(state=query_state, count=5)
        except Exception as exc:
        ...
        metadata['warm_bootstrap'] = {'seed_count': len(matrices), 'source': 'experience_replay'}
        self._warm_bootstrap_count += 1
        logger.info('WARM_BOOTSTRAP engaged: staged %d historical parameter matrices for population re-seed', len(matrices))
        return True
    ...
    def _evict_stale(self) -> None:
        now = time.monotonic()
        stale_keys = [k for k, (_, ts) in self._cache.items() if now - ts > self._ttl]
        for k in stale_keys:
            del self._cache[k]

    def route(self, metadata: Dict[str, Any], primary_strategy: Strategy, score: float, confidence_minimum: float) -> Tuple[Strategy, int]:
        self._evict_stale()
        if score >= confidence_minimum:
            return (primary_strategy, 0)
        meta_hash = hashlib.md5(json.dumps(sorted(metadata.items()), default=str).encode()).hexdigest()[:12]
        depth = 0
        for fallback in self._cascade_order:
            ...
            if fallback is Strategy.RANDOM_RESTART and self._attempt_warm_bootstrap(metadata):
                logger.info('Random restart intercepted at depth %d -> WARM_BOOTSTRAP', depth)
        ...
        if self._attempt_warm_bootstrap(metadata):
            logger.warning('Fallback cascade exhausted at depth %d — warm bootstrap restart', depth)
            return (Strategy.WARM_BOOTSTRAP, depth)
    ...
    def stats(self) -> Dict[str, Any]:
        self._evict_stale()
        return {'max_depth': self._max_depth, 'cache_size': len(self._cache), 'ttl_seconds': self._ttl, 'warm_bootstrap_count': self._warm_bootstrap_count, 'replay_buffer_attached': self._replay_buffer is not None}

def _generate_candidate_actions(metadata: Dict[str, Any], strategy: Strategy) -> List[Dict[str, Any]]:
    ...
    if strategy in (Strategy.GREEDY, Strategy.EXPLORATORY, Strategy.BALANCED):
        actions.append({'label': 'increase_exploration_depth', 'score': base_score * 1.15, 'cost': 0.08 * math.log1p(cycle)})
        actions.append({'label': 'tighten_convergence_tolerance', 'score': base_score * 1.05, 'cost': 0.05 * math.log1p(cycle)})
    if strategy in (Strategy.GREEDY, Strategy.BALANCED):
        actions.append({'label': 'boost_mutation_sigma', 'score': base_score * 1.2, 'cost': 0.12 * math.log1p(cycle)})
    if strategy in (Strategy.CONSERVATIVE, Strategy.FALLBACK_CASCADE):
        actions.append({'label': 'reduce_population_size', 'score': base_score * 0.9, 'cost': 0.02})
        actions.append({'label': 'increase_patience', 'score': base_score * 0.95, 'cost': 0.01})
    if strategy == Strategy.EXPLORATORY:
        actions.append({'label': 'random_hyperparameter_perturbation', 'score': base_score * 1.1, 'cost': 0.15})
...
def evaluate(metadata: Dict[str, Any], hyper_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stall_patience: int = int(params.get('kinetic_stall_patience', HeuristicStateMachine.KINETIC_STALL_PATIENCE))
    boost_factor: float = float(params.get('kinetic_boost_factor', 2.0))
    fsm = HeuristicStateMachine(layer_count=layer_count, velocity_epsilon=velocity_eps, acceleration_epsilon=acceleration_eps, stall_patience=stall_patience)
    ...
    signal = fsm.decide_signal(score=current_score, exploration_epsilon=exploration_eps, exploitation_bias=exploitation_bias, confidence_minimum=confidence_min)
    ...
    if kinetic_alarm:
        ...
        logger.warning('Kinetic stagnation anomaly flagged — boost_mutation_sigma signal emitted (factor=%.2f)', boost_factor)
    metadata['score_trajectory'] = [round(v, 6) for v in fsm.score_history]
    metadata['kinetic_stall_cycles'] = fsm.kinetic_stall_cycles
    state_strategy_map: Dict[str, Strategy] = {'conservative_scan': Strategy.CONSERVATIVE, 'exploratory_probe': Strategy.EXPLORATORY, 'greedy_exploit': Strategy.GREEDY, 'balanced_adaptive': Strategy.BALANCED, 'fallback_cascade': Strategy.FALLBACK_CASCADE}
    ...
    if cascade_depth > 0:
        logger.info('Fallback engaged: primary=%s -> resolved=%s (depth=%d)', primary_strategy.name, resolved_strategy.name, cascade_depth)
    ...
    if elapsed < wall_limit:
        ...
        ranked = cba.rank_actions(candidates, current_step=current_cycle, current_transitions=fsm._transition_count)
    ...
    if elapsed < wall_limit and metadata.get('ranked_actions'):
        ...
        for action in metadata['ranked_actions']:
            item = EvaluationItem(score=float(action.get('score', 0.0)), cost=float(action.get('cost', 0.0)), label=str(action.get('label', 'unknown')), payload=action)
            pq.push(item)
        best = pq.pop()
        if best is not None:
    ...
    elapsed = time.monotonic() - start_time
    if elapsed < wall_limit:
        recorded = router.record_experience(state={'current_score': current_score, 'current_cycle': current_cycle, 'fsm_state': metadata.get('fsm_state', 'none'), 'primary_strategy': metadata.get('primary_strategy', 'none'), 'resolved_strategy': metadata.get('resolved_strategy', 'none')}, action=action_label, result=current_score, parameter_matrix=metadata.get('best_config'))
